    ...
    <>
        { validate ?
            <div className="inp-control">
                ...
                {inputProps.hasOwnProperty('label') && <label className={`inp-lbl ${(touched && !isValid) || (error && !isValid) ? "lbl-error" : ""}`}>
                    {inputProps.label}
                    {inputProps.hasOwnProperty('hint') && <span className='inp-hint'>{`Hint: ${inputProps.hint}`}</span>}
                </label>}
